...
| Tdarr worker limits (the shrink flow and its libraries are regenerated by `scripts/tdarr-shrink-flow.py`) | Tdarr UI | `tdarr_configs`, `tdarr_server` | yes |
...
Out of the box Cleanuparr ran its jobs on schedule and did nothing: no
stall or slow rules, metadata and failed-import strikes at 0 (off), and
no seeding rules. What it does now, all set in its UI (or its API with
the per-user key) and stored in `.appdata/cleanuparr/cleanuparr.db`:
...
seeding rule, since nothing imported them.

Deleting the files is safe because nothing in those categories is the
only copy. Radarr, Sonarr and Lidarr import by hardlink (checked:
`copyUsingHardlinks` is on in all three), so deleting the download side
leaves the library copy. Mylar3 (`file_opts = move`) and LazyLibrarian
